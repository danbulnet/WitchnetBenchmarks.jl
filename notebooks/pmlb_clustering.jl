...
# ╔═╡ c2fe6a50-e39c-11ed-2a4d-5f297fc1f8cc
begin
	using DataFrames
	using CSV
	using Clustering
...
# ╔═╡ e7c1e5f4-8689-4ef9-8536-e1e8d43fd6e5
begin
	rootdir = normpath(joinpath(@__DIR__, ".."))
	datadir = joinpath(rootdir, "data/PMLB") |> normpath
end

# ╔═╡ c4297ac7-fbf5-4ad8-8086-85e56545d7a6
cdf = CSV.File(joinpath(datadir, "classification_statistics.csv")) |> DataFrame

# ╔═╡ 948f1a96-8157-4cde-b582-a46c97800772
rdf = CSV.File(joinpath(datadir, "regression_statistics.csv")) |> DataFrame

# ╔═╡ 812e6128-a7d8-4645-b5b7-2826d5e8e9fe
cfeatures = reshape(Float64.(cdf.features), 1, length(cdf.features))

# ╔═╡ d710b8c6-901c-4720-8775-a6db10a7839e
cfeatures_distances = pairwise(Euclidean(), cfeatures, cfeatures)

# ╔═╡ 967fd9d6-34b8-4687-83b9-22049454b74b
cfeatures_clustering = kmedoids(cfeatures_distances, 3; init=[nrow(cdf) ÷ 4, 2nrow(cdf) ÷ 4, 3nrow(cdf) ÷ 4])

# ╔═╡ c08d1e85-4e96-4119-b77a-a4a5bbeabe71
cdf.features_cluster = cfeatures_clustering.assignments

# ╔═╡ d6dfd388-1e65-4adc-90b7-b0af86e96335
plot(
	cdf, x=:features, y=:records,
...
manifest_format = "2.0"
project_hash = "5557e9ee29d33c934f4b2d9bc496e513463b72f0"
