...
# ╔═╡ 98c92180-e478-11ed-390c-cd8d25915e47
begin
	using DataFrames
	using CSV
	using Clustering
...
# ╔═╡ 7028b0c7-bf95-42dc-9771-92afdd42853e
html"""
<style>
	main {
	    max-width: 1250px;
...
# ╔═╡ 40e639e7-0dce-4f30-8f87-67390efa40e3
begin
	rootdir = normpath(joinpath(@__DIR__, ".."))
	datadir = joinpath(rootdir, "data/PMLB") |> normpath
	benchmarkdir = joinpath(rootdir, "benchmark/penn/classification") |> normpath
end

# ╔═╡ 1eee8e44-c35e-4738-97ad-05b63312a03b
function collectbenchmarks(benchmarkdir=benchmarkdir, prefix="penn_classification_")
...
begin
	cbench = collectbenchmarks();
	delete!(cbench, :magds_1k)
	length(cbench), keys(cbench)
end

# ╔═╡ 8dff5319-476a-4475-a6db-b7949e5ee17e
begin
	cbench_all_path = joinpath(
		rootdir, "benchmark/penn/penn_classification_magds_1k.csv"
	) |> normpath
	cbench_all_df = CSV.File(cbench_all_path) |> DataFrame
	index = nothing
	for (i, model) in enumerate(cbench_all_df.model)
...
begin 
	cstats_filtered = cstats[map(
		x -> Symbol(x) in keys(cbench), lowercase.(cstats.name)
	), :]
	cstats_filtered[!, :name] = Symbol.(lowercase.(cstats_filtered.name))
end
...
manifest_format = "2.0"
project_hash = "ee2471d24e8bdd7993e5f39d798b3e9f91a47ea6"
