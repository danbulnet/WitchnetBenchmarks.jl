...
# ╔═╡ 92df4330-ece3-11ed-22db-c343797d37ee
begin
	using DataFrames
	using CSV
	using Gadfly
...
# ╔═╡ 376f5587-2752-4e4c-be99-20dcbee25040
html"""
<style>
	main {
	    max-width: 1250px;
...
# ╔═╡ da9a10a0-ebc8-45af-9c8b-eb8474153df1
begin
...
function collectbenchmarks(benchmarkdir=benchmarkdir, prefix="penn_regression_")
...
begin
	nrmses = Dict{String, Vector{Float64}}()
	for model in last(first(rbench)).model
		nrmses[model] = []
	end
	for df in values(rbench)
		for row in eachrow(df)
			nrmses[row.model] = push!(nrmses[row.model], row.nrmse)
		end
	end
	nrmses
end
...
(
	...
	("ftest" => VarianceFTest(nrmses["MAGDS"], nrmses["NeuralNetworkRegressor"]) |> pvalue),
	("ttest" => UnequalVarianceTTest(nrmses["MAGDS"], nrmses["NeuralNetworkRegressor"]) |> pvalue)
)
...
manifest_format = "2.0"
project_hash = "c0bf700fb1df2c42fc1560ea1260bcee657e17c2"
